...
/**
 * Default image-generation prompt for the English workflow.
 */
export const DEFAULT_IMAGE_GENERATION_PROMPT_EN = `Using "\${summaryForImage}", generate an academic paper concept graphic that clearly presents:

Research problem: the core problem addressed
Innovative method: the main method or technique proposed
Workflow: the processing pipeline from input to output
Key results: the main experimental findings or performance gains
Application value: the practical significance of the work
Paper title: \${title}
Requirements:
**Design Guidelines (STRICTLY FOLLOW):**
1. **Art style:**
   * Modern minimalist technology infographic.
...
2. **Composition:**
   ...
   * **Background:** Clean, solid off-white or very light grey background (#F5F5F7), with no clutter.
...
3. **Color palette:**
   ...
   * High contrast with professional color grading.
4. **Text rendering:**
   * Use Times New Roman for English.
   * Main text language: \${language}.
   * The title does not need to appear in the figure.
   * All text must be clear, correctly spelled, and free of garbled characters.
5. **Negative prompt:**
   * No photorealism.
   * No messy sketches.
   * No blurry text.
 ...
 * 获取默认的生图提示词
 *
 * Chinese and English workflows use separate built-in templates so the
 * instruction language cannot leak into the generated poster.
 *
 * @returns 默认生图提示词
 */
